...
export default async function readPageHeader(databaseFile) {
  ...
  const firstFreeBlockStart = await databaseFile.read(2);
  const numberOfCells = await databaseFile.read(2);
  const startOfContentArea = await databaseFile.read(2);
  const fragmentedFreeBytes = await databaseFile.read(1);
